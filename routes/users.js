...
router.get('/list/by-date', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['admin_admins'])){
    const users = await User.find({}, default_user_list, { 'sort': { 'signedup_at': -1 }})
    if (!users) {
...
router.post('/search/by-priviledge', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['admin_admins'])){
    const divisions = await utils.parseJSON(req.body.divisions)
    if(divisions && Array.isArray(divisions)){
      const users = await User.find({'priviledges': {'$in': divisions}}, default_user_list, { 'sort': { 'signedup_at': -1 }})
      if (!users) {
...
/* GET user */
router.get('/:user_id/profile', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  // If an admin is requestinig the profile
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['admin_admins'])){
    const user = await User.findById(req.params.user_id, { 'password': 0, 'nipCode': 0 })
    if (!user) {
    ...
    if(req.user.data._id == req.params.user_id){
      ...
      if (!user) {
        res.staus(404).send('User not found')
      } else {
        res.json({ 'success': true, 'doc': user })
      }
    } else {
      const request = await Request.findOne({ 'actors': req.user.data._id }, 'has_access')
      if(request && request.has_access){
        // if the requested user allowed requesting user, give info
        const user = await User.findById(req.params.user_id, 'username profile_pic bio contact_info last_log signedup_at')
        if (!user) {
          res.status(404).send('User not found')
        } else {
          res.json({ 'success': true, 'doc': user })
        ...
        // give limited access
        const user = await User.findById(req.params.user_id, 'username bio profile_pic contact_info')
        if (!user) {
          res.status(404).send('User not found')
        } else {
...
router.post('/register', asyncMid(async (req, res, next) => {
  // Create user object
  ...
  const response = await siiauAuth.getUserInfo(req.body.nip, req.body.udgpwd)
  // Reject teachers and normies
  if(response != null && response.type !== 'P' && settings.college_centers.includes(response.campus)){
    ...
    // Check user credentials and info (I'll leave it sync right now just because i cant remember what mongoose errs I parse)
...
/* POST login user */
router.post('/login/email', asyncMid(async (req, res, next) => {
...
router.post('/login/standard', asyncMid(async (req, res, next) => {
  const user = await User.findOne({
		'username': req.body.username
	})
  if (!user){
...
router.put('/update/profile-pic', passport.authenticate('jwt', {'session': false}), utils.uploadMediaFile.single('mfile'),
  ...
  if (settings.image_mime_type.includes(req.file.mimetype)){
    ...
    const user = await User.findByIdAndUpdate(req.user.data._id, {'$set': {'profile_pic': picture}}, {'new': true})
    if (!user) {
    ...
    res.status(415).send('Unsopported media type')
  }
}))

/* PUT update bio */
router.put('/update/bio', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  // Edit profile information
  const user = await User.findByIdAndUpdate(req.user.data._id, {'$set': {'bio': req.body.bio}}, {'new':true})
  if (!user) {
    res.status(500).send('Error updating user')
  } else {
...
router.put('/update/networks', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  // Edit profile information
...
/* PUT change email */
router.put('/reset-email', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const user = await User.findById(req.user.data._id)
  if (!user) {
    res.status(404).send('User not found')
  }
  ...
  if (isMatch) {
    ...
    const user = await User.findByIdAndUpdate(req.user.data._id, { '$set': { 'email': req.body.email }})
    if (!user) {
      res.status(500).send('Error updating user')
    } else {
      res.json({ 'success': true })
...
router.put('/alias', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  ...
  if(!req.user.data.alias.handle || hours >= settings.alias_change_rate){
    ...
    const aliasHandle = (!req.body.alias || req.body.alias === '' || req.body.alias.match(/^\s*$/) !== null)?
      null : req.body.alias
    // Update user
    await User.findByIdAndUpdate(req.user.data._id,
      {
        '$set':{
    ...
    res.status(423).send('Alias is locked')
  }
}))
...
router.put('/ban', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['ban_user'])){
    const user = await User.findOneAndUpdate({ '_id': req.body.user_id, 'is_super': false },
    {
      '$set': {
        'banned': {
          'is_banned': true,
    ...
    if (!user) {
      res.status(500).send('Error pinpointing user')
    } else {
      res.json({ 'success': true })
      utils.createAndSendNotification(req.body.user_id, false, req.user.data, `You are banned`,
      'You were banned for violating the site\'s policies', { 'type': 'ban' })
    }
  } else {
    res.status(401).send('Unauthorized')
  }
...
/* PUT unban user */
router.put('/unban', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['unban_user'])){
    const user = await User.findOneAndUpdate({ '_id': req.body.user_id },
    {
      '$set': {
        'banned': {
          'is_banned': false,
...
router.put('/password', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const user = await User.findById(req.user.data._id)
  if (!user || !req.body.new_password) {
    res.status(500).send()
  }
  const isMatch = await user.comparePassword(req.body.password)
  if (isMatch) {
...
/* DELETE user */
router.delete('/remove', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['delete_user'])){
    await User.remove({ '_id': req.body.user_id, 'is_super': false })
    res.json({ 'success': true })
  } else {
    res.status(401).send('Unauthorized')
  }
...
router.post('/promote', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  if(utils.hasRequiredPriviledges(req.user.data.priviledges, ['promote_user'])){
    ...
    if (!user) {
      ...
      'You were promoted', { 'type': 'promotion'})
    }
  } else {
    res.status(401).send('Unauthorized')
  }
...
router.get('/request/:request_id', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const request = await Request.findById({ '_id': req.params.request_id, 'actors': { '$in': [req.user.data._id]}},
  'to requested_by has_access')
  if (!request) {
    res.status(404).send('Resource not found')
  } else {
...
router.get('/is-friend/:user_id', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const request = await Request.findOne({ 'actors': { '$all': [req.user.data._id, req.params.user_id]}},
  'has_access responded requested_by')
  if (!request) {
    res.status(404).send('Resource not found')
  } else {
    res.json({ 'success': true, 'doc': request })
...
router.post('/request', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  ...
  const request = await Request.findOne({ 'actors': { '$all': [req.user.data._id, newRequest.to.id]}}, 'has_access')
  if (request) {
    // A relationship has already been established
    res.json({ 'success': true, 'is_friend': request.has_access })
  } else {
    const user = await User.findOne({ '_id': newRequest.to.id }, 'username profile_pic new_requests')
    if (!user || user._id == req.user.data._id) {
      res.status(500).send()
    } else {
      if(user.new_requests < settings.max_info_requests){
        ...
        if (!request) {
          ...
          `${req.user.data.username} sent you a request`, { 'type': 'request', 'friendId': req.user.data._id })
...
router.get('/sent-requests', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const requests = await Request.find({ 'requested_by.id': req.user.data._id, 'responded': false }).select(
...
router.get('/my-requests', passport.authenticate('jwt', {'session': false}), asyncMid(async (req, res, next) => {
  const requests = await Request.find({ 'to.id': req.user.data._id, 'responded': false }).select(
    default_request_list
  ).sort(
    { 'date_requested': -1 }
  )
